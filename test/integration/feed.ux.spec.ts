...
import { FeedIndex, PrivateKey } from '@ethersphere/core-sdk'
...
test('Feed read/write as payload when data does not fit into one chunk', async () => {
  ...
  const explicitIndexReadResult = await writer.downloadPayload({ index: 0 })
  expect(explicitIndexReadResult.payload.toUtf8()).toHaveLength(5000)
})
