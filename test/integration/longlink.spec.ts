...
async function roundTrip(filename: string, content: string): Promise<void> {
  ...
  const result = await bee.upload.files(batch(), [file])
  expect(result.reference).toBeTruthy()

  const downloaded = await bee.download.file(result.reference, filename)
...
test('mixed upload: short filename + USTAR path + longlink filename', async () => {
  ...
  const result = await bee.upload.files(batch(), [short, ustar, longlink])
  expect(result.reference).toBeTruthy()

  const d1 = await bee.download.file(result.reference, 'index.html')
  expect(d1.data.toUtf8()).toBe('short content')

  const d2 = await bee.download.file(result.reference, 'images/' + 'a'.repeat(94))
  expect(d2.data.toUtf8()).toBe('ustar content')

  const d3 = await bee.download.file(result.reference, 'a'.repeat(150))
...
test('two longlink files uploaded together', async () => {
  ...
  const result = await bee.upload.files(batch(), [file1, file2])
  expect(result.reference).toBeTruthy()

  const d1 = await bee.download.file(result.reference, 'a'.repeat(150))
  expect(d1.data.toUtf8()).toBe('first file content')

  const d2 = await bee.download.file(result.reference, 'b'.repeat(150))
